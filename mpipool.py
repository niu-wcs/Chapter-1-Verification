...
class MPIPool(object):
    ...
    def map_limit(self, worker, tasks, callback=None, limits=10):
        """Evaluate a function or callable on each task in parallel using MPI, 
        but limit to a number of active tasks
        
        This is more useful when dealing with downloads from a source with active 
        connection limits, or when needing queued tasks.
        ...
        callback : callable, optional
            ...
            callback is only called on the master thread.
        limits : int, optional
            The maximum number of workers that are allowed to work on this task
            at any given time.
        ...
        while pending:
            if workerset and tasklist and (working < limits):
                worker = workerset.pop()
                taskid, task = tasklist.pop()
                log.log(_VERBOSE, "Sent task %s to worker %s with tag %s",
                        task[1], worker, taskid)
